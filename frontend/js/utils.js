...
function buildPagination(currentPage, totalPages, onPageClick, maxVisible = 5) {
    ...
    if (start > 1) {
        ...
        if (start > 2) {
            container.appendChild(btn('...', null, true));
        }
    }

    for (let i = start; i <= end; i++) {
        container.appendChild(
            btn(String(i), i, false, i === currentPage)
        );
    }
    ...
    try {
        ...
        if (path.startsWith('/videos?')) {
            ...
            const limit = parseInt(
                urlParams.get('limit') || '500',
                10
            );

            let filtered = [
                ...(typeof VIDEOS_DB !== 'undefined' ? VIDEOS_DB : [])
            ];
...
async function apiPost(path, data) {
    try {
        ...
        // Não cria fallback local para autenticação.
        // O login precisa obrigatoriamente utilizar
        // o JWT real fornecido pelo backend.
...
async function apiPut(path, data) {
    try {
        ...
        console.warn(
            'Fallback local para PUT',
            path,
            err
        );

        // Não mascarar erros de autenticação
        if (
            String(err.message).includes('401') ||
            String(err.message)
                .toLowerCase()
